...
public class Trie<T> implements TrieInterface {
    ...
    public ArrayList<String> level_arr;
    public void printLevelHelper(TrieNode<T> trieNode){
        if(trieNode != null){
            ArrayList<TrieNode<T>> g = trieNode.children;
            T temp_obj = trieNode.getValue();
            if(temp_obj != null){
                // System.out.println(ans_arr.toString());
                if(level_arr.contains(temp_obj.toString()) != true){level_arr.add(temp_obj.toString()); }
            } 
            for(int i = 0; i<g.size(); i++){
                TrieNode<T> e = g.get(i);
                	// System.out.println("-----------------");
                	// System.out.println(e.string_char);
                	// System.out.println("jdkfadfj;---------------");
                temp_obj = e.getValue();
                if(temp_obj != null) {
                	if(level_arr.contains(temp_obj.toString()) != true){level_arr.add(temp_obj.toString()); }
                } 
                if(e.children.size() != 0){
                	// System.out.println(e.children.size() + " inside piralevel helper");
                	printTrieHelper(e);
                } else{
                	temp_obj = e.getValue();
	                if(temp_obj != null) {
    ...
    public void printLevel(int level) {
        ...
        for(int i = 0; i<root.children.size(); i++){
	        ...
	        // this.printLevelHelper(root.children.get(i));
        }
